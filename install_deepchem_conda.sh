...
sudo apt update
sudo apt install -y rocm-libs miopen-hip cxlactivitylogger

# Change commented out line For amd gpu tensorflow
export tensorflow=tensorflow-rocm
...
conda install -y -q -c deepchem -c rdkit -c conda-forge -c omnia \
    ...
    biopython=1.71 \
    numpy=1.14
yes | pip install --user $tensorflow==1.13.3
